...
html = re.sub(r'  <!-- ==================== LOGIN VIEW ==================== -->.*?</section>', new_login_sections, html, flags=re.DOTALL)
...
js = js.replace('  // --- STATS COUNTERS ---', new_js)
js = re.sub(r'app\.js\?v=\d+', 'app.js?v=4', js)
...
with open('index.html', 'r', encoding='utf-8') as f:
    html = f.read()
    html = re.sub(r'app\.js\?v=\d+', 'app.js?v=4', html)
with open('index.html', 'w', encoding='utf-8') as f:
    f.write(html)
